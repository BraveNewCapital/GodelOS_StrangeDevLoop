...
try:
    from backend.core.experiment_harness import run_experiments, CONDITION_EXECUTORS
    from backend.core.statistical_analysis import run_statistical_analysis, print_summary_report
    ...
    logger.error("Make sure you're running from the GödelOS root directory with the virtual environment activated")
    exit(1)
...
class ComprehensiveExperimentRunner:
        ...
        for prompt_idx, base_prompt in enumerate(self.config["base_prompts"]):
            ...
            for condition in self.config["conditions"]:
                ...
                for run_num in range(self.config["runs_per_condition_per_prompt"]):
                    ...
                    try:
                        # Get LLM driver
                        driver = await get_llm_cognitive_driver(testing_mode=self.config["testing_mode"])
    ...
    def generate_publication_visualizations(self) -> None:
        ...
        try:
            ...
            self._create_main_results_figure(viz_data)
            self._create_depth_progression_figure(viz_data)
            self._create_condition_comparison_figure(viz_data)
            self._create_statistical_significance_figure(viz_data)
            self._create_phase_transition_figure(viz_data)
    ...
    def _create_main_results_figure(self, viz_data: Dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(self.visualization_dir / 'main_results.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _create_depth_progression_figure(self, viz_data: Dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(self.visualization_dir / 'depth_progression.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _create_condition_comparison_figure(self, viz_data: Dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(self.visualization_dir / 'condition_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _create_statistical_significance_figure(self, viz_data: Dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(self.visualization_dir / 'statistical_significance.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _create_phase_transition_figure(self, viz_data: Dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(self.visualization_dir / 'phase_transitions.png', dpi=300, bbox_inches='tight')
        plt.close()
