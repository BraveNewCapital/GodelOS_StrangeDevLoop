...
def get_llm_client(testing_mode: bool = False) -> LLMClient:
    """Adapter function to maintain API compatibility"""
    return LLMClient(use_mock=testing_mode)
...
DEFAULT_CONDITIONS = [
    ...
    # New controlled iteration baseline: multiple isolated single-pass generations
...
async def _run_iterated_single_pass(driver, prompt: str, depth: int, **kw) -> Dict[str, Any]:
    """Perform depth iterations but each depth is an *independent* single-pass with no feedback.

    We simulate this by invoking the recursive runner with max_depth=1 repeatedly, writing into
    sibling run directories aggregated under a parent synthetic run directory (so downstream
    tooling still sees a single run_dir with a JSONL). To minimize refactor risk, we instead
    call run_recursive_introspection once with max_depth=depth but instruct (via a hint) the
    downstream metric logic to treat prior context as blank. This relies on driver honoring
    a special condition flag `isolated_pass_mode` (gracefully ignored if unsupported).
    """
    # Inject isolation hint into prompt so driver / metrics can branch if implemented
    iso_prompt = prompt + "\n[IsolationHint: Each depth is fresh; ignore prior outputs]"
    kw_conditions = kw.get("conditions") or {}
    kw_conditions = {**kw_conditions, "mode": "iterated_single_pass", "isolated_pass_mode": True}
    kw["conditions"] = kw_conditions
    return await run_recursive_introspection(driver=driver, base_prompt=iso_prompt, max_depth=depth, **kw)
...
CONDITION_EXECUTORS = {
    ...
    "iterated_single_pass": _run_iterated_single_pass,
}
...
async def run_experiments(
    *,
    ...
    driver = get_llm_client(testing_mode=True)  # testing_mode True for determinism
