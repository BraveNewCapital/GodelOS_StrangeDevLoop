...
# PART 3: Using the Formal Logic Parser
    ...
    # Recommendation: In backend/production contexts, prefer using backend.core.ksi_adapter.KSIAdapter
    # or public KR endpoints (/kr/assert, /kr/retract) for mutations/queries
    # rather than direct KnowledgeStoreInterface usage.

    print("\nAdding facts to the knowledge store...")
    # Note: For backend integrations, route mutations via KSIAdapter or the public KR endpoints
    # instead of calling ksi.add_statement(...) directly.
    ...
    # Note: In backend services, add/retract rules through KSIAdapter or /kr/assert and /kr/retract
    # to ensure normalized metadata, versioning, and event streaming.
