...
from godelOS.core_kr.knowledge_store.interface import KnowledgeStoreInterface  # Note: in backend/production contexts, prefer KSIAdapter or the public KR endpoints over direct KSI
...
def main():
    ...
    ksi = KnowledgeStoreInterface(type_system)  # Prefer backend.core.ksi_adapter.KSIAdapter or public KR endpoints (/kr/assert, /kr/retract) in backend contexts
    # Prefer public KR endpoints via KRClient when the unified server is available
    use_public_endpoints = False
    kr_client = None
    if 'KRClient' in globals() and KRClient is not None:
        try:
            kr_client = KRClient()
            try:
                # Best-effort health probe to decide if backend is up
                kr_client.health()
                use_public_endpoints = True
                print("Using public KR endpoints via KRClient")
            except Exception:
                use_public_endpoints = False
        except Exception:
            use_public_endpoints = False
    ...
    if use_public_endpoints and kr_client:
        kr_client.assert_statement("Human(Socrates)", context_id="TRUTHS", fail_silently=True)
    else:
    ...
    if use_public_endpoints and kr_client:
        kr_client.assert_statement("forall ?x. Human(?x) => Mortal(?x)", context_id="TRUTHS", fail_silently=True)
    else:
    ...
    if use_public_endpoints and kr_client:
        kr_client.assert_statement("Mortal(Socrates)", context_id="TRUTHS", fail_silently=True)
    else:
