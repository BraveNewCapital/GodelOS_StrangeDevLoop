...
class Token:
    ...
    # Dependency head index (token this token depends on)
...
class LexicalAnalyzerParser:
    ...
    def _is_dependent_on(self, token: Token, head_token: Token, all_tokens: List[Token]) -> bool:
        ...
        # Determine the head index via stored metadata or heuristic fallbacks
    ...
    def _infer_head_index(self, token: Token, all_tokens: List[Token]) -> Optional[int]:
        """Infer a plausible head index when explicit dependency metadata is absent."""

        # Noun and pronoun roles typically attach to the nearest verb/root
