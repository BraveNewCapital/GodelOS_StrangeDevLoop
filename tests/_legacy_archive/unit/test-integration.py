...
def is_server_available(url: str, timeout: int = 3) -> bool:
...
async def is_websocket_available(url: str, timeout: int = 3) -> bool:
    """Check if a WebSocket server is available at the given URL."""
    try:
...
def requires_backend_server(func):
    """Decorator to skip tests if backend server is not available."""
    def wrapper(*args, **kwargs):
        if not is_server_available("http://localhost:8000/health"):
            print("⚠️ SKIP: Backend server not available (run: uvicorn backend.unified_server:app --port 8000)")
            return True  # Return True to not fail the test suite
        return func(*args, **kwargs)
    return wrapper

def requires_websocket_server(func):
    """Decorator to skip tests if WebSocket server is not available."""
    async def async_wrapper(*args, **kwargs):
        if not await is_websocket_available("ws://localhost:8000/ws/unified-cognitive-stream"):
            print("⚠️ SKIP: WebSocket server not available (run: uvicorn backend.unified_server:app --port 8000)")
            return True  # Return True to not fail the test suite
