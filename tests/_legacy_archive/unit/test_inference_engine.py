...
def test_inference_engine():
    ...
    try:
        # Create unification engine first
        unification_engine = UnificationEngine()
        
        # Create a knowledge base with the unification engine
        kb = InMemoryKnowledgeStore(unification_engine)
        ...
        a = ConstantNode(name="a", type_ref=entity_type)
        P = ApplicationNode(name="P", arity=1, type_ref=boolean_type)
        fact = P(a)
